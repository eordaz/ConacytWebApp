...
function validarLetras(e) { // 1
    ...
    keychar = String.fromCharCode(key);
    var validos = "abcdefghijklmnñopqrstuvwxyzABCDEFGHIJKLMNÑOPQRSTUVWXYZáéíóúÁÉÍÓÚ ";
    if ((key == null) || (key == 0) || (key == 8) || (key == 9) || (key == 13) || (key == 27)) { return true; }
    if (validos.indexOf(keychar) > -1)
        return true;
    else
...
function validarLetrasCopiar(id) {
    var cadena = document.getElementById(id).value;
   var validos = "abcdefghijklmnñopqrstuvwxyzABCDEFGHIJKLMNÑOPQRSTUVWXYZáéíóúÁÉÍÓÚÄËÏÖÜ ";
    var erroneos = "";
    var i;
    for (i = 0; i < cadena.length; i++) {
        if (!(validos.indexOf(cadena.charAt(i)) > -1)) {
